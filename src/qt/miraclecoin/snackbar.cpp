...
#include "qt/miraclecoin/forms/ui_snackbar.h"
#include "qt/miraclecoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(MiracleCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &MiracleCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
