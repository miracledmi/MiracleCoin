...
#include "qt/miraclecoin/dashboardwidget.h"
#include "qt/miraclecoin/forms/ui_dashboardwidget.h"
#include "qt/miraclecoin/sendconfirmdialog.h"
#include "qt/miraclecoin/txrow.h"
...
DashboardWidget::DashboardWidget(MiracleCoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	setCssProperty(ui->labelSquareMrc, "square-chart-mrc");
	setCssProperty(ui->labelMrc, "text-chart-mrc");
	...
	setCssProperty(ui->labelAmountMrc, "text-stake-mrc-disable");
	...
	if (window)
		connect(window, &MiracleCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
	if (walletModel && walletModel->getOptionsModel()) {
		...
		connect(ui->pushImgEmpty, &QPushButton::clicked, window, &MiracleCoinGUI::openFAQ);
		connect(ui->btnHowTo, &QPushButton::clicked, window, &MiracleCoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	chartData->amountsByCache = getAmountBy(); // pair MRC
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			mrc = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalMrc += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesMrc.append(mrc);

		int max = std::max(mrc, mrc);
...
void DashboardWidget::onChartRefreshed()
{
	...
	set0->setColor(QColor(201, 16, 90));
	...
	set0->append(chartData->valuesMrc);

	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalMrc > 0) {
		setCssProperty(ui->labelAmountMrc, "text-stake-mrc");
	}
	else {
		setCssProperty(ui->labelAmountMrc, "text-stake-mrc-disable");
	}
	forceUpdateStyle({ ui->labelAmountMrc });
	ui->labelAmountMrc->setText(GUIUtil::formatBalance(chartData->totalMrc, nDisplayUnit));
