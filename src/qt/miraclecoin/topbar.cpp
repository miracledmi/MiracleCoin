...
#include "qt/miraclecoin/topbar.h"
#include "qt/miraclecoin/forms/ui_topbar.h"
#include "qt/miraclecoin/lockunlock.h"
#include "qt/miraclecoin/qtutils.h"
#include "qt/miraclecoin/receivedialog.h"
#include "qt/miraclecoin/loadingdialog.h"
...
TopBar::TopBar(MiracleCoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopMrc}, "amount-small-topbar");
    setCssProperty({ui->labelAmountMrc}, "amount-topbar");
    setCssProperty({ui->labelPendingMrc, ui->labelImmatureMrc}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("MiracleCoin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // MRC Total
    QString totalMrc = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);

    // MRC
    // Top
    ui->labelAmountTopMrc->setText(totalMrc);
    // Expanded
    ui->labelAmountMrc->setText(totalMrc);
    ui->labelPendingMrc->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureMrc->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
