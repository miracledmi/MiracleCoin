...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter MiracleCoin address (e.g. %1)").arg("MJJFMPGnacSnJrYgvPpA4JuiQKmBUkqvPN"));
...
{
    // return if URI is not valid or is no MiracleCoin: URI
...
{
    // Convert miraclecoin:// to miraclecoin:
    //
    //    Cannot handle this later, because miraclecoin:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::MRC, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MiracleCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for MiracleCoin.lnk
...
{
    return GetAutostartDir() / "miraclecoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a miraclecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=MiracleCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the miraclecoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add miraclecoin app to startup item list
