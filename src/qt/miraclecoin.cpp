...
#include "config/miraclecoin-config.h"
#endif
...
#include "qt/miraclecoin/splash.h"
#include "qt/miraclecoin/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("miraclecoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in miraclecoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in miraclecoin.qrc)
...
/** Class encapsulating MiracleCoin startup and shutdown.
...
/** Main MiracleCoin application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (MiracleCoinGUI)
...
{
    window = new MiracleCoinGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &MiracleCoinGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &MiracleCoinGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(MiracleCoinGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(MiracleCoinGUI::DEFAULT_WALLET);
        ...
        // MiracleCoin: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MiracleCoinGUI::handlePaymentRequest);
        connect(window, &MiracleCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. MiracleCoin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(miraclecoin_locale);
    Q_INIT_RESOURCE(miraclecoin);
    ...
    /// 6. Determine availability of data directory and parse miraclecoin.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("MiracleCoin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("MiracleCoin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // miraclecoin: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("MiracleCoin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
