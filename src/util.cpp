...
const char * const MiracleCoin_CONF_FILENAME = "miraclecoin.conf";
const char * const MiracleCoin_PID_FILENAME = "miraclecoin.pid";
const char * const MiracleCoin_MASTERNODE_CONF_FILENAME = "masternode.conf";


// MiracleCoin only features
// Masternode
...
#else
    const char* pszModule = "miraclecoin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\MiracleCoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\MiracleCoin
// Mac: ~/Library/Application Support/MiracleCoin
// Unix: ~/.miraclecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MiracleCoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MiracleCoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MiracleCoinParams
    // Mac: ~/Library/Application Support/MiracleCoinParams
    // Unix: ~/.miraclecoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MiracleCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "MiracleCoinParams";
#else
    // Unix
    return pathRet / ".miraclecoin-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", MiracleCoin_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", MiracleCoin_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty miraclecoin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override miraclecoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", MiracleCoin_PID_FILENAME));
