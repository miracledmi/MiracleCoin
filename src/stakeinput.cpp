...
bool CMrcStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CMrcStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CMrcStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CMrcStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CMrcStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CMrcStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CMrcStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CMrcStake::GetUniqueness() const
{
    //The unique identifier for a MRC stake is the outpoint
...
CBlockIndex* CMrcStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CMrcStake::ContextCheck(int nHeight, uint32_t nTime)
{
