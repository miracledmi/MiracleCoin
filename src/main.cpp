...
#if defined(NDEBUG)
#error "MiracleCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in umrc) are considered zero fee (for relaying, mining and transaction creation)
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 366666866) {
		nSubsidy = 0.06 * COIN;
	}
	else if (nHeight == 366666867)
		nSubsidy = 0.04 * COIN;
	else
		nSubsidy = 0 * COIN;
...
{
    util::ThreadRename("miraclecoin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> mrcInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                mrcInputs.push_back(stakeIn);
        }
        const bool hasMRCInputs = !mrcInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasMRCInputs) {
                    ...
                    for (const CTxIn& mrcIn : mrcInputs)
                        if(mrcIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasMRCInputs) {
                            for (const CTxIn& stakeIn : mrcInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // MiracleCoin: We use certain sporks during IBD, so check to see if they are
