...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/MiracleCoinParams"
    else
        PARAMS_DIR="$HOME/.miraclecoin-params"
    fi
fi
...
Failed to fetch the MiracleCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
MiracleCoin - fetch-params.sh

This script will fetch the MiracleCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common MiracleCoin zkSNARK parameters. Note that it is
