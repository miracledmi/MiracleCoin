...
from test_framework.test_framework import MiracleCoinTestFramework
...
class ReorgStakeTest(MiracleCoinTestFramework):
    ...
    def check_money_supply(self, expected_bc, expected_zmrc):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected MRC and zMRC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bc))
            for denom in node['zMRCsupply']:
                assert_equal(node['zMRCsupply'][denom], DecimalAmt(expected_zmrc[denom]))
    ...
    def run_test(self):
        ...
        # Check MRC and zMRC supply at the beginning
        # ------------------------------------------
        # zMRC supply: 2 coins for each denomination
        expected_zmrc_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # MRC supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zmrc_supply)
        ...
        # Verify that MRC and zMRC supplies were properly updated after the spends and reorgs
        self.log.info("Check MRC and zMRC supply...")
        ...
        expected_zmrc_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zmrc_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zmrc_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zmrc_supply)
