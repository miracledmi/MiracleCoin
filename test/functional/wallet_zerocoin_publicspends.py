...
class ZerocoinSpendTest(MiracleCoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zmrc_bal, bc_bal):
            zmrc_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zmrc_bal)
            bc_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], bc_bal)
            return zmrc_bal, bc_bal
        ...
        zmrc_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zmrc_balance, 6666)
        ...
        zmrc_balance, balance = check_balances(denom_2, zmrc_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zmrc_balance, balance = check_balances(denom_3, zmrc_balance, balance)
        ...
        _, _ = check_balances(denom_1, zmrc_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
